...
class MultipleOnResultIdTargets {
    ...
    @Test
    fun sendExceptions() {
        Navigator.config(ConfigurationField.MULTIPLE_ON_RESULT_ID_TARGETS, MultipleOnResultIdTargets.SEND_ERROR)

        Assert.assertThrows(TooManyTargetsException::class.java) {
            activityRule.scenario.onActivity {
                Navigate.toReturn("tooReturnTargets").andThen()
            }
        }
        Assert.assertThrows(TooManyTargetsException::class.java) {
            activityRule.scenario.onActivity {
                Navigate.toReturn("tooReturnTargets").andThen("tooReturnTargets")
            }
        }
    }
    @Test
    fun noSendExceptions() {
        Navigator.config(ConfigurationField.MULTIPLE_ON_RESULT_ID_TARGETS, MultipleOnResultIdTargets.PICK_FIRST)
