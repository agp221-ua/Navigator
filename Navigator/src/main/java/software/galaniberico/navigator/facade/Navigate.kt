...
object Navigate {
    ...
    inline fun <reified T : Any?> get(id: String, default: T? = null): T? {
        ...
        if (value !is T) throw IllegalStateException("The retrieved data is not of the expected type.")
        return value
    }
    ...
    fun load(currentActivity: Activity){
        if(NavigatorConfigurations.unloadNavigateData != UnloadNavigateData.FROM_MANUAL_LOAD_UNTIL_MANUAL_NULLIFY)
            throw IllegalStateException("Calling this method with the current configuration is not allowed.")
        val activityId = currentActivity.intent.getStringExtra(Facade.getIdKey())
            ?: throw IllegalArgumentException("The provided Activity does not contain a valid ID. It is possible that it was not started by this plugin.")
        if (NavigateDataManager.isLoaded())
            throw IllegalStateException("Attempting to load navigation data while another one is still loaded. Please nullify the previous one before loading a new one.")
