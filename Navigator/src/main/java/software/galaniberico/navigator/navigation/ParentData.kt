...
private var data: MutableMap<String, Any?>? = null
    ...
    fun saveData() {
        ...
        for (field in activity!!::class.java.declaredFields){
            if (NavigatorConfigurations.parentActivityMapProtocol.view && View::class.java.isAssignableFrom(field.type)) continue
            if (NavigatorConfigurations.parentActivityMapProtocol.context && Context::class.java.isAssignableFrom(field.type)) continue
