...
class NavigationManager internal constructor(var activity: Activity?) {
    ...
    private fun resolveParentData(activity: Activity): ParentData {
        ...
        if (NavigatorConfigurations.parentActivityDataAccess.saveActivity) parentData.activity = activity
        if (NavigatorConfigurations.parentActivityDataAccess != ParentActivityDataAccess.MAP_COPY) return parentData
        parentData.saveData()
        return parentData
    }
